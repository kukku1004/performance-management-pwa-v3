...
export default function MeetingCalendar({ notes, members, selectedMemberId, open, onToggle }: { notes: MeetingNote[]; members: TeamMember[]; selectedMemberId: string; open: boolean; onToggle: () => void }) {
  ...
  const upcomingNotes = scheduled.filter((note) => note.date >= todayValue)
  ...
  if (!open) return <div className="m-2 flex min-w-0 flex-col gap-2 self-start text-center"><button type="button" onClick={() => { void syncGoogleCalendar() }} disabled={syncing} className="ui-button ui-button-secondary ui-button-sm w-full px-1 text-[10px]" title="Google Calendar에 면담 일정 연동">{syncing ? '연동 중' : '↻ 일정 연동'}</button><button type="button" onClick={onToggle} title="면담 일정 펼치기" aria-label={`면담 일정 펼치기 · 예정 ${upcomingNotes.length}건`} className="flex w-full min-w-0 flex-col items-center rounded-lg border border-gray-200 bg-white px-1 py-3 text-center text-gray-600 hover:bg-gray-50 hover:text-gray-950"><span className="inline-flex items-center gap-1 text-[11px] font-semibold"><svg aria-hidden="true" viewBox="0 0 24 24" className="h-3.5 w-3.5 fill-none stroke-current" strokeWidth="1.8"><path d="M6 3v3M18 3v3M4 8h16M5 5h14v15H5z"/></svg>면담</span><span className="mt-1 text-[9px] text-gray-400">예정 {upcomingNotes.length}건</span>{upcomingNotes.length > 0 ? <span className="mt-2 flex max-h-72 w-full flex-col gap-2 overflow-y-auto px-1">{upcomingNotes.map((note) => <span key={note.id} className="flex w-full min-w-0 flex-col items-center border-t border-gray-100 pt-2 first:border-t-0 first:pt-0"><span className="text-[10px] font-semibold tabular-nums text-gray-700">{note.date.slice(5).replace('-', '/')}</span><span className="mt-0.5 flex max-w-full items-center gap-1 text-[9px] text-gray-500"><span className="h-1.5 w-1.5 shrink-0 rounded-full" style={{ backgroundColor: memberColor(note.memberId) }} /><span className="truncate">{memberMap.get(note.memberId)?.name ?? '팀원'}</span></span></span>)}</span> : <span className="mt-2 text-[9px] text-gray-400">일정 없음</span>}</button>{syncMessage && <p className="break-words text-[9px] leading-3 text-gray-400">{syncMessage}</p>}</div>
