    ...
    <div ref={layoutRef} className={`meeting-focus-workspace ${referencePanelsMinimized ? 'meeting-focus-workspace-expanded' : ''}`} style={{ gridTemplateColumns: referencePanelsMinimized
      ...
      <main className="meeting-focus-document">
        ...
        <section className="mt-6">
          ...
          <div className="mt-3 flex items-stretch gap-4"><textarea value={newComment} onChange={(event) => onCommentChange(event.target.value)} rows={7} placeholder="면담 내용을 입력하세요." className="ui-field min-w-0 flex-1 resize-y" /><div className="flex w-[188px] shrink-0 flex-col justify-between gap-3"><div className="grid grid-cols-3 gap-2" aria-label="면담 분위기">{MOODS.map((mood) => { const selected = newMood === mood.value; return <button key={mood.value} type="button" title={mood.label} aria-label={mood.label} aria-pressed={selected} onClick={() => onMoodChange(selected ? '' : mood.value)} className={`flex h-11 w-11 items-center justify-center rounded-md border transition ${selected ? 'scale-105 border-current' : 'border-transparent hover:bg-gray-50'}`} style={{ color: mood.color, backgroundColor: selected ? `${mood.color}14` : undefined }}><MoodGlyph value={mood.value} className="h-6 w-6" /></button> })}</div>{loadedNote ? <div className="flex gap-2"><button type="button" onClick={() => onUpdateLoaded(loadedNote)} disabled={!newComment.trim()} className="ui-button ui-button-primary min-w-0 flex-1 px-2">수정하기</button><button type="button" onClick={() => onDelete(loadedNote)} className="ui-button ui-button-danger shrink-0 px-3">삭제</button></div> : <button type="button" onClick={onAdd} disabled={!newComment.trim()} className="ui-button ui-button-primary w-full">작성하기</button>}</div></div>
        ...
        {insights.length > 0 && <section className="mt-5 overflow-hidden rounded-lg border border-gray-200 bg-white">
          ...
          {insightsOpen && <div className="divide-y divide-gray-100 border-t border-gray-100">{insights.map((insight) => <button key={insight.id} type="button" onClick={() => setSelectedInsight(insight)} className="flex w-full items-center gap-3 px-4 py-3 text-left hover:bg-gray-50"><span className={`h-2 w-2 shrink-0 rounded-full ${insight.tone === 'positive' ? 'bg-emerald-500' : insight.tone === 'attention' ? 'bg-orange-500' : 'bg-gray-400'}`} /><span className="min-w-0 flex-1 truncate text-sm font-medium text-gray-900">{insight.title}</span><span className="shrink-0 text-xs font-semibold text-accent">상세</span><svg aria-hidden="true" viewBox="0 0 20 20" className="h-4 w-4 shrink-0 fill-current text-gray-400"><path d="m7.5 4.5 5 5.5-5 5.5 1.3 1.2 6.1-6.7-6.1-6.7z" /></svg></button>)}</div>}
        </section>}
