    ...
    <div ref={layoutRef} className={`meeting-focus-workspace ${referencePanelsMinimized ? 'meeting-focus-workspace-expanded' : ''}`} style={{ gridTemplateColumns: !referencePanelsMinimized
      ...
      <main className="meeting-focus-document meeting-focus-document-row">
        ...
        <section className="mt-7">
          ...
          {commentRecordsOpen && (allCommentRecords.length === 0 ? <p className="py-5 text-sm text-gray-400">가져온 성과 코멘트가 없습니다.</p> : <div className="divide-y divide-gray-100">{allCommentRecords.map((record) => <details key={record.id} open className="group/period py-4"><summary className="flex cursor-pointer list-none flex-wrap items-center gap-2 text-left"><DisclosureIcon open className="h-4 w-4 shrink-0 text-gray-400 transition-transform group-open/period:rotate-0" /><strong className="text-sm text-gray-950">{record.period}</strong><span className="text-xs text-gray-400">{record.comments.length}건</span>{record.fileName && <span className="ml-auto max-w-72 truncate text-xs text-gray-400">{record.fileName}</span>}<span className="ml-1 text-xs font-medium text-gray-400 group-open/period:hidden">펼치기</span><span className="ml-1 hidden text-xs font-medium text-gray-400 group-open/period:inline">접기</span></summary><div className="mt-3 space-y-2 pl-6">{record.comments.map((comment, index) => <div key={`${record.id}-${index}`} className="flex items-start gap-1"><details className="group min-w-0 flex-1 rounded-md border border-gray-200 px-3 py-2"><summary className="flex cursor-pointer list-none items-center gap-2 text-sm text-gray-700"><span className="shrink-0 text-xs font-semibold text-gray-500">코멘트 {index + 1}</span><span className="min-w-0 flex-1 truncate group-open:hidden">{comment}</span><span className="ml-auto text-xs font-medium text-gray-400 group-open:hidden">펼쳐보기</span><span className="ml-auto hidden text-xs font-medium text-gray-400 group-open:inline">접기</span></summary><p className="mt-2 whitespace-pre-wrap border-t border-gray-100 pt-2 text-sm leading-6 text-gray-700">{comment}</p></details><button type="button" onClick={() => setCommentToDelete({ recordId: record.id, commentIndex: index })} className="ui-button ui-button-danger ui-button-sm h-9 w-9 shrink-0 px-0" title="코멘트 삭제" aria-label={`${record.period} 코멘트 ${index + 1} 삭제`}><svg aria-hidden="true" viewBox="0 0 24 24" className="h-4 w-4 fill-none stroke-current" strokeWidth="1.8"><path d="M4 7h16M9 7V4h6v3M7 7l1 13h8l1-13M10 11v5M14 11v5" /></svg></button></div>)}</div></details>)}</div>)}
        </section>
