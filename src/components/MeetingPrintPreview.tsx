...
const OPTION_LABELS: Array<[PrintOption, string]> = [
  ...
  ['lastMeeting', '지난 면담 요약'], ['growth', '육성 포인트 작성란'], ['draft', '작성 중인 면담 내용 포함'], ['memo', '면담 내용 영역'],
]
...
export default function MeetingPrintPreview({ member, history, insights, latestNote, draft, growthPoints, onClose }: { member: TeamMember; history: MemberEvaluationHistory[]; insights: MemberInsight[]; latestNote: MeetingNote | null; draft: string; growthPoints: { strength: string; improvement: string; challenge: string; careerGoal: string }; onClose: () => void }) {
  ...
  const [pageLayout, setPageLayout] = useState<'vertical' | 'horizontal'>('vertical')
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const height = Math.max(400, element.clientHeight - verticalPadding - (extraPages > 0 && pageLayout === 'vertical' ? 48 : 0))
      const pageCount = 1 + extraPages
      const contentWidth = pageLayout === 'horizontal' ? 680 * pageCount + 16 * extraPages : 680
      setPreviewScale(Math.min(width / contentWidth, height / 962))
    }
    update()
    const observer = new ResizeObserver(update)
    observer.observe(element)
    return () => observer.disconnect()
  }, [extraPages, pageLayout])
  ...
  function firstPageHtml(totalPages: number) {
    ...
    ${options.performance ? `<section class="status-section"><div class="summary"><strong>${latest ? `${escapeHtml(latest.label)} · ${latest.score.toFixed(1)}점 · ${latest.grade}` : '성과 데이터 없음'}</strong>${latest && previous ? `<span>직전 대비 <b class="delta-value">${(latest.score - previous.score) >= 0 ? '+' : ''}${(latest.score - previous.score).toFixed(1)}점</b></span>` : ''}</div></section>` : ''}
  ...
  function blankPageHtml(page: number, totalPages: number) {
    return `<div class="print-page blank-page">${pageHeader(page, totalPages)}${options.memo ? '<section class="memo-section"><h2>면담 내용</h2><div class="writing-lines blank-lines"></div></section>' : ''}</div>`
  }
