...
type PrintOption = 'profile' | 'performance' | 'insights' | 'questions' | 'evidence' | 'lastMeeting' | 'growth' | 'draft' | 'memo' | 'memoLines'
...
const DEFAULT_OPTIONS: Record<PrintOption, boolean> = { profile: true, performance: true, insights: true, questions: true, evidence: false, lastMeeting: true, growth: false, draft: false, memo: true, memoLines: true }
...
export default function MeetingPrintPreview({ member, history, insights, latestNote, draft, growthPoints, onClose }: { member: TeamMember; history: MemberEvaluationHistory[]; insights: MemberInsight[]; latestNote: MeetingNote | null; draft: string; growthPoints: { strength: string; improvement: string; challenge: string; careerGoal: string }; onClose: () => void }) {
  ...
  function pageHeader(page: number, totalPages: number) {
    return `<header><div><p class="eyebrow">1:1 면담 준비지</p><h1>${escapeHtml(member.name)}</h1><p>${escapeHtml(member.level || '직급 미설정')} · ${member.yearsOfService ?? '-'}년차</p></div><p>면담일 ${!blankDate && printDate ? escapeHtml(printDate.replace(/-/g, '. ')) : '__________________'}</p></header>${totalPages > 1 ? `<span class="page-number">${page} / ${totalPages}</span>` : ''}`
  }
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-black/25 p-2"><section role="dialog" aria-modal="true" aria-labelledby="meeting-print-title" className="flex h-[calc(100vh-16px)] w-full max-w-6xl flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg">
    <header className="flex h-12 shrink-0 items-center justify-between border-b border-gray-200 px-4"><div className="flex items-baseline gap-3"><h2 id="meeting-print-title" className="text-[16px] font-semibold text-gray-950">면담용지 출력하기</h2><p className="text-[13px] text-gray-500">출력할 항목만 선택</p></div><ModalCloseButton onClick={onClose} label="면담용지 출력하기 닫기" /></header>
    <div className="grid min-h-0 flex-1 lg:grid-cols-[minmax(0,1fr)_260px]">
      <main className="min-h-0 overflow-y-auto overflow-x-hidden bg-gray-200 p-3"><div className="meeting-print-sheet space-y-4" dangerouslySetInnerHTML={{ __html: `<style>.meeting-print-sheet .print-page{position:relative;width:min(780px,100%);aspect-ratio:210/297;margin:0 auto;background:#fff;padding:32px 38px;color:#111827;font-size:11px;line-height:20px;box-shadow:0 1px 3px rgb(0 0 0 / 10%)}.meeting-print-sheet header{display:flex;justify-content:space-between;align-items:flex-end;border-bottom:2px solid #111827;padding-bottom:10px}.meeting-print-sheet h1{margin:2px 0;font-size:22px;font-weight:700}.meeting-print-sheet .eyebrow{font-size:10px;font-weight:700;color:#ea580c}.meeting-print-sheet .page-number{position:absolute;right:38px;bottom:22px;color:#6b7280;font-size:10px}.meeting-print-sheet section{margin-top:14px}.meeting-print-sheet h2{margin:0 0 7px;border-bottom:1px solid #d1d5db;padding-bottom:4px;font-size:12px;font-weight:700}.meeting-print-sheet .growth-title span{margin-left:8px;color:#9ca3af;font-size:10px;font-weight:400}.meeting-print-sheet .growth-content{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px}.meeting-print-sheet .growth-item{border:1px solid #e5e7eb;padding:7px}.meeting-print-sheet .growth-item p,.meeting-print-sheet .meeting-draft{margin:4px 0;white-space:pre-wrap}.meeting-print-sheet .growth-writing{height:30px;border-bottom:1px solid #d1d5db}.meeting-print-sheet .summary{display:flex;justify-content:space-between}.meeting-print-sheet .insight{padding:5px 0}.meeting-print-sheet .insight>p{margin:2px 0 0;color:#4b5563}.meeting-print-sheet .evidence{margin:4px 0 0;border-left:2px solid #fed7aa;padding-left:7px;color:#6b7280;font-size:10px}.meeting-print-sheet .evidence p{margin:1px 0}.meeting-print-sheet .evidence span{font-weight:700}.meeting-print-sheet .line,.meeting-print-sheet .text{margin:4px 0;white-space:pre-wrap}.meeting-print-sheet .memo-section{margin-top:18px}.meeting-print-sheet .writing-lines{height:260px;background:repeating-linear-gradient(to bottom,transparent 0,transparent 24px,#d1d5db 25px)}.meeting-print-sheet .blank-writing-space{height:260px}.meeting-print-sheet .blank-lines{height:calc(100% - 95px)}</style>${pagesHtml()}` }} /></main>
      <aside className="flex min-h-0 flex-col border-l border-gray-200 bg-gray-50"><div className="flex-1 overflow-y-auto p-4"><p className="text-xs font-semibold text-gray-500">면담일</p><input type="date" value={printDate} onChange={(event) => { setPrintDate(event.target.value); setBlankDate(false) }} disabled={blankDate} className="ui-field ui-field-sm mt-2 bg-white" /><label className="mt-2 flex cursor-pointer items-center gap-2 text-xs text-gray-600"><input type="checkbox" checked={blankDate} onChange={(event) => setBlankDate(event.target.checked)} className="h-4 w-4 accent-orange-600" />빈 날짜로 출력</label><div className="mt-5 border-t border-gray-200 pt-4"><p className="text-xs font-semibold text-gray-500">출력 항목</p><div className="mt-2 space-y-1">{OPTION_LABELS.map(([key, label]) => <label key={key} className="flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-sm text-gray-700 hover:bg-white"><input type="checkbox" checked={options[key]} onChange={() => toggle(key)} className="h-4 w-4 accent-orange-600" />{label}</label>)}</div></div><div className="mt-5 border-t border-gray-200 pt-4"><p className="text-xs font-semibold text-gray-500">빈 면담 페이지</p><p className="mt-1 text-xs leading-5 text-gray-400">메모 공간이 더 필요하면 추가하세요.</p><div className="mt-3 flex items-center justify-between rounded-md border border-gray-200 bg-white p-2"><button type="button" onClick={() => setExtraPages((count) => Math.max(0, count - 1))} disabled={extraPages === 0} className="ui-button ui-button-ghost ui-button-sm h-8 w-8 px-0">−</button><strong className="text-sm tabular-nums text-gray-900">{extraPages}장 추가</strong><button type="button" onClick={() => setExtraPages((count) => Math.min(2, count + 1))} disabled={extraPages === 2} className="ui-button ui-button-ghost ui-button-sm h-8 w-8 px-0">+</button></div></div></div><div className="flex gap-2 border-t border-gray-200 bg-white p-4"><button type="button" onClick={onClose} className="ui-button ui-button-secondary min-w-0 flex-1">취소</button><button type="button" onClick={print} className="ui-button ui-button-primary min-w-0 flex-1">인쇄</button></div></aside>
    </div>
  </section></div>
}
