...
export default function MeetingPrintPreview({ member, history, insights, latestNote, draft, growthPoints, onClose }: { member: TeamMember; history: MemberEvaluationHistory[]; insights: MemberInsight[]; latestNote: MeetingNote | null; draft: string; growthPoints: { strength: string; improvement: string; challenge: string; careerGoal: string }; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    if (!element) return
    const update = () => {
      const width = Math.max(320, element.clientWidth - 24)
      const height = Math.max(400, element.clientHeight - (extraPages > 0 ? 72 : 24))
      setPreviewScale(Math.min(1, width / 680, height / 962))
    }
    update()
  ...
  function pageHeader(page: number, totalPages: number) {
    return `<header><div><h1>${escapeHtml(member.name)}</h1><p>${escapeHtml(member.level || '직급 미설정')} · ${member.yearsOfService ?? '-'}년차</p></div><p>면담일 ${!blankDate && printDate ? escapeHtml(printDate.replace(/-/g, '. ')) : '__________________'}</p></header>${totalPages > 1 ? `<span class="page-number">${page} / ${totalPages}</span>` : ''}`
  }
