...
export default function MemberGrowthOverview({ member, collapsedContent, onPanelMinimizedChange, hideSummary = false, removeTopSpacing = false, simulationOnRight = false, simulationTriggerContainerId, showPerformancePanel = true }: { member: TeamMember; compact?: boolean; collapsible?: boolean; collapsedContent?: ReactNode; onPanelMinimizedChange?: (bothMinimized: boolean) => void; hideSummary?: boolean; removeTopSpacing?: boolean; simulationOnRight?: boolean; simulationTriggerContainerId?: string; showPerformancePanel?: boolean }) {
  ...
  const simulationBonusRows = simulation.rows.flatMap((row) => {
    ...
    return Math.abs(bonus) < 0.1 ? [] : [{ year: row.year, bonus, weighted: row.weighted }]
  })
    ...
    <section className={showPerformancePanel ? 'flex min-h-full flex-col' : 'h-0'}>
      ...
      <div className={`${collapsedContent ? 'flex flex-1 bg-slate-50' : 'bg-white'}`}>
        <div ref={panelRef} style={collapsedContent && !simulationPanelMinimized ? { left: simulationPopupPosition.x, top: simulationPopupPosition.y, width: criteriaOpen ? 'min(1480px, calc(100vw - 48px))' : 'min(760px, calc(100vw - 48px))', height: criteriaOpen ? 'calc(100vh - 32px)' : undefined, maxHeight: 'calc(100vh - 32px)' } : undefined} className={simulationPanelMinimized ? (collapsedContent ? 'hidden' : 'bg-transparent') : collapsedContent ? `fixed z-[60] rounded-lg border border-slate-200 bg-white p-4 shadow-xl transition-[width] duration-200 ${criteriaOpen ? 'overflow-hidden' : 'overflow-y-auto'}` : `space-y-5 bg-white ${removeTopSpacing ? 'pt-0' : 'pt-6'} px-0`}>
          ...
          <div className="grid grid-cols-[112px_minmax(210px,1fr)_minmax(180px,0.9fr)] overflow-hidden rounded-xl border border-slate-200 bg-white"><div className="px-3 py-2.5"><p className="text-[11px] font-medium text-slate-500">목표 점수</p><strong className="mt-1 block text-lg tabular-nums text-slate-950">{simulation.targetScore.toFixed(1)}점</strong></div><div className="border-x border-slate-200 px-3 py-2.5"><p className="text-[11px] font-medium text-slate-500">현재 점수 + 시뮬레이션 가산</p><div className="mt-1 flex items-baseline gap-2"><strong className="text-lg tabular-nums text-slate-950">{currentSimulation.currentScore.toFixed(1)}점</strong><span className="font-semibold text-blue-600">+ {simulationBonus.toFixed(1)}점</span></div></div><div className={`px-3 py-2.5 ${expectedGap >= 0 ? 'border-emerald-500' : 'border-orange-500'}`}><p className="text-[11px] font-medium text-slate-500">최종 시뮬레이션 점수</p><strong className={`mt-1 block text-lg tabular-nums ${expectedGap >= 0 ? 'text-emerald-600' : 'text-orange-600'}`}>{simulation.currentScore.toFixed(1)}점</strong></div></div>
          <div className="flex flex-wrap items-center gap-1.5 text-[11px] text-gray-600"><strong className="mr-1 text-gray-800">가산 근거</strong>{simulationBonusRows.length ? simulationBonusRows.map((item) => <span key={item.year} className="rounded-md border border-gray-200 bg-white px-2 py-1">{item.year}년 가산 {item.bonus >= 0 ? '+' : ''}{item.bonus.toFixed(1)} · 가중합 {item.weighted.toFixed(1)}점</span>) : <span className="text-gray-400">추가 입력으로 반영된 가산점이 없습니다.</span>}</div>
