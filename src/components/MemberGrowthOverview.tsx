...
export default function MemberGrowthOverview({ member, collapsedContent, onPanelMinimizedChange, hideSummary = false, removeTopSpacing = false, simulationOnRight = false, simulationTriggerContainerId, showPerformancePanel = true }: { member: TeamMember; compact?: boolean; collapsible?: boolean; collapsedContent?: ReactNode; onPanelMinimizedChange?: (bothMinimized: boolean) => void; hideSummary?: boolean; removeTopSpacing?: boolean; simulationOnRight?: boolean; simulationTriggerContainerId?: string; showPerformancePanel?: boolean }) {
  ...
  const [simulationStep, setSimulationStep] = useState<'history' | 'auxiliary'>('history')
    ...
    <section className={showPerformancePanel ? 'flex min-h-full flex-col' : 'h-0'}>
      ...
      <div className={`${collapsedContent ? 'flex flex-1 bg-slate-50' : 'bg-white'}`}>
        <div ref={panelRef} style={collapsedContent && !simulationPanelMinimized ? { left: simulationPopupPosition.x, top: simulationPopupPosition.y, width: criteriaOpen ? 'min(1240px, calc(100vw - 48px))' : 'min(560px, calc(100vw - 48px))', maxHeight: 'calc(100vh - 32px)' } : undefined} className={simulationPanelMinimized ? (collapsedContent ? 'hidden' : 'bg-transparent') : collapsedContent ? `simulation-popup-compact fixed z-[60] overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-xl transition-[width] duration-200` : `space-y-5 bg-white ${removeTopSpacing ? 'pt-0' : 'pt-6'} px-0`}>
          ...
          <div className="grid grid-cols-2 rounded-lg bg-slate-100 p-1" role="tablist" aria-label="승진 시뮬레이션 상세">
            <button type="button" role="tab" aria-selected={simulationStep === 'history'} onClick={() => setSimulationStep('history')} className={`rounded-md px-3 py-2 text-sm font-semibold transition ${simulationStep === 'history' ? 'bg-white text-slate-950 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}>평가 이력</button>
            <button type="button" role="tab" aria-selected={simulationStep === 'auxiliary'} onClick={() => setSimulationStep('auxiliary')} className={`rounded-md px-3 py-2 text-sm font-semibold transition ${simulationStep === 'auxiliary' ? 'bg-white text-slate-950 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}>보조지표</button>
          </div>
          {simulationStep === 'history' && <>
          ...
          <div className="flex items-center justify-between border-t border-slate-200 pt-3">
            <button type="button" disabled={simulationStep === 'history'} onClick={() => setSimulationStep('history')} className="ui-button ui-button-secondary ui-button-sm disabled:invisible">이전</button>
            <span className="text-xs font-medium text-slate-400">{simulationStep === 'history' ? '1 / 2' : '2 / 2'}</span>
            <button type="button" onClick={() => setSimulationStep(simulationStep === 'history' ? 'auxiliary' : 'history')} className="ui-button ui-button-secondary ui-button-sm">{simulationStep === 'history' ? '다음: 보조지표' : '평가 이력으로'}</button>
          </div>
