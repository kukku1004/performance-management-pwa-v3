...
import { applyMemberTenure, auxiliaryMetricsFromPersonnelRecord, calculatePromotionSimulation, getDefaultGrowthProfile, getMemberEvaluationHistory, GRADE_POINTS, mergeProjectHistoryForSimulation } from '../utils/growth'
...
export default function MemberGrowthOverview({ member, collapsedContent, onPanelMinimizedChange, hideSummary = false, removeTopSpacing = false, simulationOnRight = false, simulationTriggerContainerId, showPerformancePanel = true }: { member: TeamMember; compact?: boolean; collapsible?: boolean; collapsedContent?: ReactNode; onPanelMinimizedChange?: (bothMinimized: boolean) => void; hideSummary?: boolean; removeTopSpacing?: boolean; simulationOnRight?: boolean; simulationTriggerContainerId?: string; showPerformancePanel?: boolean }) {
  ...
  const [auxiliaryDetailOpen, setAuxiliaryDetailOpen] = useState<'position' | 'rewardPenalty' | 'tenure' | 'education' | null>(null)
  ...
  useEffect(() => {
    if (!member.personnelRecord || profile.auxiliaryMetrics?.source === 'manual') return
    const personnelMetrics = auxiliaryMetricsFromPersonnelRecord(member)
    const current = profile.auxiliaryMetrics
    if (current?.source === 'personnel-record'
      && current.position === personnelMetrics.position
      && current.rewardPenalty === personnelMetrics.rewardPenalty
      && current.tenure === personnelMetrics.tenure
      && current.education === personnelMetrics.education) return
    updateProfile({ auxiliaryMetrics: personnelMetrics })
  }, [member.personnelRecord, member.position, member.yearsOfService, profile.auxiliaryMetrics])
  useEffect(() => {
  ...
  function updateAuxiliary(key: 'position' | 'rewardPenalty' | 'tenure' | 'education', value: string) {
    updateProfile({ auxiliaryMetrics: { ...(profile.auxiliaryMetrics ?? { position: 0, rewardPenalty: 0, tenure: 0, education: 0 }), [key]: Number(value) || 0, ...(key === 'tenure' ? { tenureOverridden: true } : {}), source: 'manual' } })
  }
        ...
        <div ref={panelRef} style={collapsedContent && !simulationPanelMinimized ? { left: simulationPopupPosition.x, top: simulationPopupPosition.y, width: criteriaOpen ? 'min(1240px, calc(100vw - 48px))' : 'min(560px, calc(100vw - 48px))', maxHeight: 'calc(100vh - 32px)' } : undefined} className={simulationPanelMinimized ? (collapsedContent ? 'hidden' : 'bg-transparent') : collapsedContent ? `simulation-popup-compact fixed z-[60] overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-xl transition-[width] duration-200` : `space-y-5 bg-white ${removeTopSpacing ? 'pt-0' : 'pt-6'} px-0`}>
          ...
          <p className="text-xs leading-5 text-slate-500">{reviewLabel} 심사 기준으로 {firstYear ?? '-'}년부터 {lastYear ?? '-'}년까지의 5년 데이터를 반영합니다.</p>
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 px-5 py-4"><div className="flex items-center gap-3"><h4 className="ui-section-title">보조지표</h4><strong className="text-sm">합계 {simulation.auxiliaryScore}점</strong></div><div className="mt-3 grid grid-cols-2 gap-x-6 gap-y-2">{([['position', '직책'], ['rewardPenalty', '상벌'], ['tenure', '체류'], ['education', '교육']] as const).map(([key, label]) => {
              ...
              const sourceLabel = profile.auxiliaryMetrics?.source === 'personnel-record' ? '개별 인사기록카드 Excel' : profile.auxiliaryMetrics?.source === 'manual' ? '사용자 수정값 (인사기록카드 기준)' : profile.auxiliaryMetrics?.source === 'growth-file' ? '인사평가·승진 시뮬레이션 Excel' : '기본값'
              return <div key={key} className="relative grid grid-cols-[40px_64px] items-center gap-2"><span className="flex items-center gap-1 text-xs font-medium text-gray-600">{label}<button type="button" onClick={() => setAuxiliaryDetailOpen((value) => value === key ? null : key)} aria-expanded={auxiliaryDetailOpen === key} aria-label={`${label} 산정 근거 보기`} title={`${label} 산정 근거`} className="inline-flex h-5 w-5 items-center justify-center rounded-full text-orange-600 hover:bg-orange-100"><svg aria-hidden="true" viewBox="0 0 24 24" className="h-3.5 w-3.5 fill-none stroke-current" strokeWidth="1.8"><circle cx="12" cy="12" r="9"/><path d="M12 11v5M12 8h.01"/></svg></button></span><input type="number" aria-label={`${label} 보조점수`} value={key === 'tenure' ? effectiveTenure : (profile.auxiliaryMetrics?.[key] ?? 0)} onChange={(event) => updateAuxiliary(key, event.target.value)} title={key === 'tenure' && hasManagedTenure && !hasTenureOverride ? '팀원 관리의 연차를 기본값으로 적용했습니다. 수정할 수 있습니다.' : undefined} className="ui-field ui-field-sm w-16 bg-white text-right" />{auxiliaryDetailOpen === key && <div role="dialog" aria-label={`${label} 산정 근거`} className="absolute bottom-full left-0 z-30 mb-2 w-80 rounded-lg border border-gray-200 bg-white p-3 shadow-lg"><div className="mb-2 flex items-center justify-between"><strong className="text-sm text-gray-900">{label} 산정 근거{(key === 'education' || key === 'rewardPenalty') ? ` ${details.length}건` : ''}</strong><button type="button" onClick={() => setAuxiliaryDetailOpen(null)} aria-label="산정 근거 닫기" className="ui-icon-button h-6 w-6">×</button></div><p className="rounded-md bg-orange-50 px-2.5 py-2 text-xs font-medium text-orange-800">출처: {sourceLabel}</p>{key === 'position' && <p className="mt-2 text-xs leading-5 text-gray-600">인사기록 직책: {member.personnelRecord?.duty || member.position || '정보 없음'} · 적용 점수 {(profile.auxiliaryMetrics?.position ?? 0)}점</p>}{key === 'tenure' && <p className="mt-2 text-xs leading-5 text-gray-600">팀원 관리 연차: {member.yearsOfService ?? '정보 없음'}년 · 적용 점수 {effectiveTenure}점</p>}{(key === 'education' || key === 'rewardPenalty') && <div className="mt-2 divide-y divide-gray-100">{details.length === 0 ? <p className="py-2 text-xs text-gray-500">저장된 상세 내역이 없습니다. 적용 점수는 {profile.auxiliaryMetrics?.[key] ?? 0}점입니다.</p> : key === 'education' ? member.personnelRecord!.education.map((item, index) => <div key={`${item.startDate}-${index}`} className="py-2 text-xs"><p className="font-medium text-gray-800">{item.courseName || '교육명 없음'}</p><p className="mt-0.5 text-gray-500">{item.startDate || '-'}{item.endDate ? ` ~ ${item.endDate}` : ''}{item.score ? ` · ${item.score}` : ''}</p></div>) : member.personnelRecord!.awards.map((item, index) => <div key={`${item.date}-${index}`} className="py-2 text-xs"><p className="font-medium text-gray-800">{item.name || '상벌명 없음'}</p><p className="mt-0.5 text-gray-500">{item.date || '-'}{item.organization ? ` · ${item.organization}` : ''}</p>{item.reason && <p className="mt-1 leading-5 text-gray-600">{item.reason}</p>}</div>)}</div>}</div>}</div>
            })}</div><p className={`mt-3 text-xs leading-5 ${hasManagedTenure || hasTenureOverride ? 'text-slate-500' : 'text-orange-700'}`}>{profile.auxiliaryMetrics?.source === 'personnel-record' ? '개별 인사기록카드 Excel의 직책·상벌·체류·교육 정보를 우선 적용했습니다. 각 값은 여기서 수정할 수 있습니다.' : profile.auxiliaryMetrics?.source === 'manual' ? '개별 인사기록카드 기준값을 사용자가 수정한 상태입니다. 다른 성과·평가 파일을 올려도 유지됩니다.' : hasTenureOverride ? `체류 ${effectiveTenure}년을 시뮬레이션 값으로 직접 적용했습니다.` : hasManagedTenure ? `체류는 팀원 관리의 연차 ${member.yearsOfService}년을 기본 적용하며 여기서 수정할 수 있습니다.` : '팀원 관리에 연차가 없어 Excel의 체류 값을 적용했습니다. Excel 값은 실제 정보와 다를 수 있으니 반드시 확인해 주세요.'}</p></div>
