...
export default function GrowthHistoryImportDialog({ members, profiles, onApply, onClose }: {
  ...
  async function loadFiles(files: FileList | File[]) {
    ...
    setActiveMemberName(documents.find((item) => item.document.comments.length > 0)?.document.memberName ?? '')
    setFileNames(loadedNames)
    setResult(parsed)
    setMessage(parsed.importedMembers.length > 0 ? '' : '현재 팀원과 일치하는 성과 이력을 찾지 못했습니다.')
  }

  const commentDocuments = performanceDocuments.filter((item) => item.document.comments.length > 0)
  const commentMemberNames = Array.from(new Set(commentDocuments.map((item) => item.document.memberName)))
  const selectedMemberName = commentMemberNames.includes(activeMemberName) ? activeMemberName : (commentMemberNames[0] ?? '')
  const activeDocuments = commentDocuments.filter((item) => item.document.memberName === selectedMemberName)
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/25 p-4">
    <div role="dialog" aria-modal="true" aria-labelledby="growth-history-import-title" className="flex h-[min(900px,calc(100vh-2rem))] w-full max-w-5xl flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg">
      ...
      <div className="flex min-h-0 flex-1 flex-col p-5">
        {!result ? <><button type="button" onClick={() => inputRef.current?.click()} onDragOver={(event) => { event.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); void loadFiles(event.dataTransfer.files) }} className={`flex flex-1 flex-col items-center justify-center rounded-lg border border-dashed px-5 py-10 text-center transition ${dragging ? 'border-accent bg-orange-50' : 'border-gray-300 bg-gray-50 hover:border-accent hover:bg-white'}`}><svg aria-hidden="true" viewBox="0 0 24 24" className="h-6 w-6 fill-none stroke-gray-400" strokeWidth="1.7"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6M8 14h8M8 18h5"/></svg><span className="mt-3 text-sm font-medium text-gray-800">클릭하거나 파일을 끌어다 놓으세요</span><span className="mt-1 text-xs text-gray-400">여러 이전 성과 Excel·성과평가 PDF를 한 번에 선택할 수 있습니다.</span></button><input ref={inputRef} type="file" multiple accept=".xlsx,.xls,.pdf,application/pdf" className="hidden" onChange={(event) => { if (event.target.files) void loadFiles(event.target.files); event.target.value = '' }} /></> : <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-gray-200"><div className="flex items-center justify-between gap-3 border-b border-gray-100 px-4 py-3"><div className="min-w-0"><p className="truncate text-sm font-medium text-gray-900">{fileNames.length}개 파일 · 팀원 {result.importedMembers.length}명</p><p className="mt-1 text-xs text-gray-500">{commentDocuments.length > 0 ? '팀원 탭을 선택해 기간별 코멘트를 확인하세요.' : '연결할 성과 이력을 확인했습니다.'}</p></div><button type="button" onClick={() => { setResult(null); setFileNames([]); setMessage(''); setPerformanceDocuments([]); setActiveMemberName('') }} className="ui-button ui-button-ghost ui-button-sm">다른 파일</button></div>{commentMemberNames.length > 0 && <div className="flex shrink-0 gap-1 overflow-x-auto border-b border-gray-200 bg-gray-50 px-4 pt-3" role="tablist" aria-label="팀원별 성과 코멘트">{commentMemberNames.map((name) => { const memberDocuments = commentDocuments.filter((item) => item.document.memberName === name); const selectedCount = memberDocuments.reduce((sum, item) => sum + item.selectedComments.length, 0); const totalCount = memberDocuments.reduce((sum, item) => sum + item.document.comments.length, 0); return <button key={name} type="button" role="tab" aria-selected={selectedMemberName === name} onClick={() => setActiveMemberName(name)} className={`shrink-0 rounded-t-md border border-b-0 px-4 py-2.5 text-sm font-semibold ${selectedMemberName === name ? 'border-gray-300 bg-white text-gray-950' : 'border-transparent text-gray-500 hover:text-gray-800'}`}>{name}<span className="ml-2 text-xs font-medium text-gray-400">{selectedCount}/{totalCount}</span></button> })}</div>}<div className="min-h-0 flex-1 overflow-y-auto p-5">{activeDocuments.map((activeDocument) => <section key={activeDocument.document.id} className="mb-5 last:mb-0"><div className="flex items-center justify-between gap-3 border-b border-gray-100 pb-3"><div className="min-w-0"><h3 className="text-base font-semibold text-gray-950">{activeDocument.document.periodLabel}</h3><p className="mt-1 truncate text-xs text-gray-500">{activeDocument.document.fileName}</p></div><span className="shrink-0 text-sm font-medium text-accent">{activeDocument.selectedComments.length}/{activeDocument.document.comments.length}개 선택</span></div><div className="mt-3"><PerformanceCommentSelector comments={activeDocument.document.comments} selected={activeDocument.selectedComments} onToggle={(index) => setPerformanceDocuments((current) => current.map((item) => item.document.id !== activeDocument.document.id ? item : { ...item, selectedComments: item.selectedComments.includes(index) ? item.selectedComments.filter((value) => value !== index) : [...item.selectedComments, index] }))} /></div></section>)}</div></div>}
