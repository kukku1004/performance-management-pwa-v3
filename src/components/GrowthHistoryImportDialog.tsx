...
export default function GrowthHistoryImportDialog({ members, profiles, onApply, onClose }: {
  ...
  const activeDocument = commentDocuments[Math.min(activeDocumentIndex, Math.max(0, commentDocuments.length - 1))]
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/25 p-4">
    <div role="dialog" aria-modal="true" aria-labelledby="growth-history-import-title" className="flex max-h-[calc(100vh-2rem)] w-full max-w-xl flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-5">
        {!result ? <><button type="button" onClick={() => inputRef.current?.click()} onDragOver={(event) => { event.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); void loadFiles(event.dataTransfer.files) }} className={`flex w-full flex-col items-center justify-center rounded-lg border border-dashed px-5 py-10 text-center transition ${dragging ? 'border-accent bg-orange-50' : 'border-gray-300 bg-gray-50 hover:border-accent hover:bg-white'}`}><svg aria-hidden="true" viewBox="0 0 24 24" className="h-6 w-6 fill-none stroke-gray-400" strokeWidth="1.7"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6M8 14h8M8 18h5"/></svg><span className="mt-3 text-sm font-medium text-gray-800">클릭하거나 파일을 끌어다 놓으세요</span><span className="mt-1 text-xs text-gray-400">여러 이전 성과 Excel·성과평가 PDF를 한 번에 선택할 수 있습니다.</span></button><input ref={inputRef} type="file" multiple accept=".xlsx,.xls,.pdf,application/pdf" className="hidden" onChange={(event) => { if (event.target.files) void loadFiles(event.target.files); event.target.value = '' }} /></> : <div className="rounded-lg border border-gray-200"><div className="flex items-center justify-between gap-3 border-b border-gray-100 px-4 py-3"><div className="min-w-0"><p className="truncate text-sm font-medium text-gray-900">{fileNames.length}개 파일 · 팀원 {result.importedMembers.length}명</p><p className="mt-1 text-xs text-gray-500">{commentDocuments.length > 0 ? `코멘트 확인 ${Math.min(activeDocumentIndex + 1, commentDocuments.length)} / ${commentDocuments.length}` : '연결할 성과 이력을 확인했습니다.'}</p></div><button type="button" onClick={() => { setResult(null); setFileNames([]); setMessage(''); setPerformanceDocuments([]); setActiveDocumentIndex(0) }} className="ui-button ui-button-ghost ui-button-sm">다른 파일</button></div>{activeDocument && <section className="p-4"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><h3 className="truncate text-sm font-semibold text-gray-950">{activeDocument.document.memberName}</h3><p className="mt-1 truncate text-xs text-gray-500">{activeDocument.document.periodLabel} · {activeDocument.document.fileName}</p></div><span className="shrink-0 text-xs font-medium text-accent">{activeDocument.selectedComments.length}/{activeDocument.document.comments.length}개 선택</span></div><div className="mt-3 max-h-72 overflow-y-auto"><PerformanceCommentSelector comments={activeDocument.document.comments} selected={activeDocument.selectedComments} onToggle={(index) => setPerformanceDocuments((current) => current.map((item) => item.document.id !== activeDocument.document.id ? item : { ...item, selectedComments: item.selectedComments.includes(index) ? item.selectedComments.filter((value) => value !== index) : [...item.selectedComments, index] }))} /></div></section>}</div>}
        ...
        <div className="mt-4 flex items-center justify-between gap-3"><button type="button" onClick={() => void downloadQuickStartTemplateFile('growth')} className="ui-button ui-button-ghost">입력 양식 받기</button><div className="flex gap-2">{result && commentDocuments.length > 1 && activeDocumentIndex > 0 && <button type="button" onClick={() => setActiveDocumentIndex((index) => Math.max(0, index - 1))} className="ui-button ui-button-secondary">이전</button>}<button type="button" onClick={onClose} className="ui-button ui-button-secondary">취소</button>{result && activeDocumentIndex < commentDocuments.length - 1 ? <button type="button" onClick={() => setActiveDocumentIndex((index) => Math.min(commentDocuments.length - 1, index + 1))} className="ui-button ui-button-primary">다음 파일</button> : <button type="button" disabled={!result || result.importedMembers.length === 0} onClick={apply} className="ui-button ui-button-primary">성과 이력 적용</button>}</div></div>
      </div>
    </div>
  </div>
}
