...
import type { ImportedPerformanceDocument, MeetingNote, MemberGrowthProfile, TeamMember } from '../types'
...
export default function GrowthHistoryImportDialog({ members, profiles, onApply, onClose }: {
  ...
  onApply: (profiles: MemberGrowthProfile[], meetingNote?: MeetingNote) => void
  onClose: () => void
}) {
  ...
  function apply() {
    ...
    const member = performanceDocument
      ...
      : undefined
    const meetingNote = performanceDocument && member
      ? createPerformanceMeetingNote(performanceDocument, member.id, performanceDocument.comments.filter((_, index) => selectedComments.includes(index))) ?? undefined
      : undefined
    onApply(result.profiles, meetingNote)
    onClose()
  }
