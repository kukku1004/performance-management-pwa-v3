...
export default function PeerReviewSection() {
  ...
  const taskParticipants = useMemo(() => state.members.filter((member) => state.contributions.some((item) => item.taskId === receivedTaskId && item.memberId === member.id && item.contributionPercent > 0)), [receivedTaskId, state.contributions, state.members])
