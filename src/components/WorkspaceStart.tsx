...
function ProjectCard({ project, onOpen, onEdit, onDelete }: { project: EvaluationProject; onOpen: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return <article className="group overflow-hidden rounded-lg border border-gray-200 bg-white transition hover:border-gray-300 hover:shadow-sm"><button type="button" onClick={onOpen} className="block w-full px-4 pb-3 pt-4 text-left"><div className="flex items-start justify-between gap-3"><div><h4 className="text-base font-semibold text-gray-950">{formatEvaluationPeriod(project.period)}</h4><p className="mt-1 text-xs text-gray-500">최근 수정 {new Date(project.updatedAt).toLocaleDateString('ko-KR')}</p></div><span className="text-xs text-gray-400">팀원 {project.appState.members.length}명</span></div><div className="mt-4 flex min-h-9 items-center gap-1.5">{members.length > 0 ? <>{members.map((member) => <span key={member.id} title={`${member.name} · ${member.level || '직급 미설정'}`} className="flex h-8 w-8 items-center justify-center rounded-full border border-gray-200 bg-gray-50 text-xs font-semibold text-gray-700">{member.name.slice(0, 1)}</span>)}{remaining > 0 && <span className="ml-1 text-xs font-medium text-gray-500">+{remaining}</span>}</> : <span className="text-sm text-gray-400">등록된 팀원 없음</span>}</div></button><div className="flex items-center justify-between border-t border-gray-100 px-4 py-2"><span className="text-xs text-gray-400">프로젝트 열기</span><div className="flex gap-1"><button type="button" onClick={onEdit} className="ui-button ui-button-ghost ui-button-sm">수정</button><button type="button" onClick={onDelete} className="ui-button ui-button-ghost ui-button-sm text-danger">삭제</button></div></div></article>
}
